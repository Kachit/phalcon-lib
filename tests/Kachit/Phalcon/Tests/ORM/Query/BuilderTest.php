...
use Kachit\Phalcon\Testable\ORM\Query\BuilderTestable;
...
class BuilderTest extends TestCase {
    ...
    public function testCountSimple() {
        ...
        $this->assertInstanceOf('Kachit\Phalcon\ORM\Query\Builder', $result);
        $this->assertEquals('COUNT (*)', $result->getColumns());
        $this->assertEquals('SELECT COUNT (*) FROM [Kachit\Phalcon\Testable\ORM\Query\BuilderTestable]', $result->getPhql());
    }

    public function testCountWithAlias() {
        $result = $this->testable->count(null, 'foo');
        $this->assertEquals('COUNT (*) AS foo', $result->getColumns());
        $this->assertEquals('SELECT COUNT (*) AS foo FROM [Kachit\Phalcon\Testable\ORM\Query\BuilderTestable]', $result->getPhql());
    }
    ...
    public function testCountWithFieldAndAlias() {
        ...
        $this->assertEquals('SELECT COUNT (id) AS foo FROM [Kachit\Phalcon\Testable\ORM\Query\BuilderTestable]', $result->getPhql());
    }

    public function testSumSimple() {
        $result = $this->testable->sum('price');
        $this->assertEquals('SUM (price)', $result->getColumns());
        $this->assertEquals('SELECT SUM (price) FROM [Kachit\Phalcon\Testable\ORM\Query\BuilderTestable]', $result->getPhql());
    }

    public function testAverageSimple() {
        $result = $this->testable->average('price');
        $this->assertEquals('AVG (price)', $result->getColumns());
        $this->assertEquals('SELECT AVG (price) FROM [Kachit\Phalcon\Testable\ORM\Query\BuilderTestable]', $result->getPhql());
    }

    public function testMaxSimple() {
        $result = $this->testable->maximum('id');
        $this->assertEquals('MAX (id)', $result->getColumns());
        $this->assertEquals('SELECT MAX (id) FROM [Kachit\Phalcon\Testable\ORM\Query\BuilderTestable]', $result->getPhql());
    }

    public function testMinSimple() {
        $result = $this->testable->minimum('id');
        $this->assertEquals('MIN (id)', $result->getColumns());
        $this->assertEquals('SELECT MIN (id) FROM [Kachit\Phalcon\Testable\ORM\Query\BuilderTestable]', $result->getPhql());
    }
}
